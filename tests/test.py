...
from zillion.core import TableTypes, UnsupportedGrainException, InvalidFieldException
...
class TestZillion(TestBase):
    ...
    def getWarehouse(self):
        return Warehouse(
            self.datasources, config=self.config, ds_priority=self.ds_priority
        )
    ...
    def testColumnConfigOverride(self):
        ...
        wh = self.getWarehouse()
        self.assertNotIn(
            "sales.lead_id", wh.dimensions["lead_id"].get_column_names("testdb1")
        )
    ...
    def testReportRollup(self):
        ...
        rollup = ROLLUP_TOTALS
        result = wh.execute(
            facts, dimensions=dimensions, criteria=criteria, rollup=rollup
        )
